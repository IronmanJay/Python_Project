...
class Example(Frame):
    ...
    def initUI(self):
        ...
        self.abtn = Button(self, text="Open", command=self.onOpen)
        self.abtn.grid(row=1, column=self.textColumn +1)
        ...
        self.abtn.bind('<Return>', self.returnEnter)
        ...
        if filename:
            ...
            for i in filename[::-1]:
                if '9' >= i >= '0':
                    ...
                    break
            fileName = str(int(index) + 1) + '.txt'
        else:
            fileName = ""
        dlg = Filedialog.Open(self, filetypes = ftypes, initialfile = fileName)
        # file_opt = options =  {}
