...
class Bar:
    ...
    def get_pos(self, step, max_value):
        ...
        if self.rank != self.lastrank:#有位置变化 需要计算位移和平滑数字
            ...
            if self.rank > TOP_NUM and self.lastrank <= TOP_NUM:#掉出榜面
                alpha = 255 * (1-step/DATE_INTERVAL)
                show = True
            elif self.rank <= TOP_NUM and self.lastrank > TOP_NUM:#入榜
...
pygame.init()
screen = pygame.display.set_mode((1380,720)) #暂定
